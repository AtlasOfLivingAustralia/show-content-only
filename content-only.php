<?php
/*
Plugin Name: Show Content Only (ALA)
Plugin URI: https://github.com/AtlasOfLivingAustralia/show-content-only
Description: Display only the post's content, without a theme, scripts or stylesheets.
Author: Katz Web Services, Inc. and Atlas of Living Australia
...
class ShowContentOnly {
	...
	function keyword() {
		...
		if ( isset( $_GET['content-only'] ) && is_singular() ) {
			the_post();
			$headreplace = array("%id%" => $post->ID, "%title%" => $post->post_title);
			echo strtr('<html><head><meta name="robots" value="noindex,nofollow" /><meta name="id" content="%id%" /><title>%title%</title>', $headreplace);
			if ( ! empty( $_GET['js'] ) ) {
				wp_print_scripts();
			}
			if ( ! empty( $_GET['css'] ) ) {
